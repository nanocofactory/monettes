...
Scripts are embedded in monet diagrams as path polyominoes (two-dimensional diagram partitions) marked with different colors. Scripts are in general multi-indexed by one or more indices, such subscripts or scenes/sequels (depending on them being signals/responses) are rendered in script/path polyominoes as strings (one-dimensional polyomino partitions) labelled with different index (sub)sets.
